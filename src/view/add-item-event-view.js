...
const createPointAddTemplate = (tripPoint) => {
  ...
  const eventTypeLabel = offerType.charAt(0).toUpperCase() + offerType.slice(1);

  const addableOffersMarkup = createOffersTemplateMarkup(offers);

  const eventTypesMarkup = createTypesMarkup(offers(), offerType);
